...
4. [Known Data Sources (Discovery)](#known-data-sources-discovery)
5. [Proposed Solution](#proposed-solution)
6. [Anomaly Taxonomy](#anomaly-taxonomy)
7. [Expected Value](#expected-value)
8. [High-Level Conceptual Architecture](#high-level-conceptual-architecture)
9. [Open Decisions (To Be Solutioned)](#open-decisions-to-be-solutioned)
10. [Roadmap](#roadmap)
...
## Known Data Sources (Discovery)

Two Adobe Analytics data feeds are currently in scope for Phase 1 discovery. Their **likely Azure landing points** — based on current provisional understanding and subject to confirmation with the data-platform owners — are captured below. These are anchor points for solutioning; nothing here is a committed integration.

### Canada Retirement (GWAM)

| Layer | Value |
|---|---|
| Storage | ADLS Gen2 / Enterprise Data Lake |
| Format | Delta / Parquet files |
| Catalog | Unity Catalog (external or managed table) |
| Query surface | Databricks SQL |
| Fully-qualified name | `gwam_prod_catalog.inv_typed_common.adobe_hit_data` |

```text
ADLS Gen2 / Enterprise Data Lake
  → Delta / Parquet files
  → Unity Catalog (external or managed table)
  → Databricks SQL
  → gwam_prod_catalog.inv_typed_common.adobe_hit_data
```
...
|---|---|
| Storage | ADLS Gen2 / Enterprise Data Lake |
| Catalog | External table or external view |
| Query surface | Synapse serverless SQL |
| Fully-qualified name | `martech.adobe_coverme.hit_data` |

```text
ADLS Gen2 / Enterprise Data Lake
  → External table or external view
  → Synapse serverless SQL
  → martech.adobe_coverme.hit_data
```

Local samples of these feeds live under `data/`. Column-level profiles are in [docs/discovery/](docs/discovery/) — see [data_inventory.md](docs/discovery/data_inventory.md) for narrative context, [data_profiling_report.md](docs/discovery/data_profiling_report.md) for the deep column-level profile, and [data_profile_summary.json](docs/discovery/data_profile_summary.json) for the machine-readable summary.

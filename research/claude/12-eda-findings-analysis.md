...
1. **The data is a thin slice of a huge table.** The source holds **3.18 B** GWAM Adobe hit rows across **1,198 columns / ~357 GB**; CA Retirement is **1,159,010 rows (0.036 %)**, selected by `rsid = manulifeglobalprod` **AND** page‑URL prefix `…/ca/en/personal/group-plans/group-retirement`. ↺ *2026‑08‑04: this URL‑prefix definition is no longer the alerting scope — doc‑16 **D13** redefines Canada Retirement as the SME's 16 link rules at qualified‑visit grain ([21](21-gwam-link-rule-scope.md)); see §10 assumption 2.*
...
- **Scope is URL‑bound.** In the profiling window, `rsid`‑only matches = **7,683,413**, but `url`‑only = both = **1,165,431**. The **URL prefix is the binding constraint**; `manulifeglobalprod` alone is far broader (mobile app, other suites). `manulifeglobalprod` is **not** a top‑10 report suite (top rsids: `manufingbrsmobileapp.prod` 56.9 %, `jhfswamjhreupeprod` 26.6 %, `manugrs` 7.8 %). ↺ *These shares are for this EDA's profiling window only. The full‑history census (probe C1, 2026‑07‑29, 3.25B rows over 16 rsids) gives `manufingbrsmobileapp.prod` **68.9 %**, `jhfswamjhreupeprod` 14.6 %, `manugrs` 9.9 % — see [19 §0](19-gwam-channel-readiness.md).*
...
2. CA Retirement = `rsid = manulifeglobalprod` **AND** URL contains `…/ca/en/personal/group-plans/group-retirement` (both notebook widgets).
   > ↺ **2026-07-29 — a candidate replacement for this definition now exists, and the rsid half is confirmed.** The SME defines Canada Retirement by the **eVar105 brand tag** (`ca-retirement` + `GWAM`, matched on parts) rather than by URL; the ruling that scope is the Public Website only (doc-16 **D11**) also confirms `manulifeglobalprod` as the sole suite. **The URL definition above is still what the code does** — switching is a full re-baseline and is not decided ([20](20-gwam-sme-questions.md) Q3; ↺ **Q3b, on the two then-unclassified brand variants, was answered 2026-07-30 — `wealth-ca` and `pvt-wealth` are both OUT of Canada Retirement, so the eVar105 parts-match is confirmed as written and this paragraph's uncertainty is now Q3 alone**). Every percentage in this document was computed under the URL definition; the two disagree on ~4% of rows, so figures here are not automatically transferable if the switch happens.
   > ↺ **2026-08-04 — both definitions above are superseded for the alerting scope by D13.** Doc-16 **D13** ([21](21-gwam-link-rule-scope.md)) redefines Canada Retirement as the SME's **16 link rules at qualified-visit grain** — neither the URL prefix nor the eVar105 brand tag is the scope definition any more (the tag survives only as a qualifier inside the rule set). Probe C13–C18 (run 2026-08-05) measured the qualified-visit population at ~25% of the URL scope (median 1,599 qualified visits/day, accepted as the phase-1 re-baseline). Figures in this document describe the URL-scoped population and do **not** transfer to the qualified-visit series.

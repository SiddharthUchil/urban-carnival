...
13. 🚩 **Port `numerator`/`denominator` + governance fields from `CmSeriesSpec` to GWAM's `SeriesSpec`** ([registry.py:68-83](../../detect/registry.py)). `gold_lib` already resolves `ratio` series by sibling `metric_id` — no engine change needed. Safe to do now; depends on no ruling. [19 §3](19-gwam-channel-readiness.md) G2. ↺ **PROMOTED to critical path 2026-07-29:** it used to block only "Sign in % rate completion", which has since left scope — but D11g's `gwam_pw_pv_per_visit` is a per-visit ratio, so this is now the one engineering gate standing between us and a metric the SME actually asked for.

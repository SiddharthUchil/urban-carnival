...
11. **Run `eda/gwam_channel_discovery.py`** (written 2026-07-28, never executed). Ten open questions in [19 §2](19-gwam-channel-readiness.md) are answerable from data, not from the SME — rsid existence/history, web-vs-app per suite, the eVar105 value + delimiter + **segment-vs-URL scope sizing** (the D10 decision input), eVar185-vs-eVar110 (also closes backlog #7's Platform half), error/sign-in field population, per-suite event ids. Check `run_manifest.skipped == {}`.
12. **Locate the `GRS+` and `manucustomer.prod` report suites.** Two of four D9 channels cannot be found in the source table. If they sit in a different Adobe instance this is a data-access request, not a build task — start it early. [20](20-gwam-sme-questions.md) Q2, Q10.
